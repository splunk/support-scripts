...
class CertificateVerifier:
        ...
        try:
            ...
            try:
                result = subprocess.run(
                    [str(self.splunk_home / "bin" / "splunk"), "version"],
                    capture_output=True,
                    text=True,
                    timeout=30,
                )
    ...
    def parse_server_conf(self) -> Dict:
        ...
        try:
            ...
            btool_cmd = [
                str(self.splunk_home / "bin" / "splunk"),
                "btool",
                "server",
                "list",
            ]
            ...
            result = subprocess.run(
                btool_cmd, capture_output=True, text=True, timeout=60
            )

            if result.returncode == 0:
                self.log_info(
                    "Successfully retrieved effective server.conf using btool"
                )
                if self.verbose:
                    self.log_debug(
                        f"btool output length: {len(result.stdout)} characters"
                    )
                ...
                self.log_warning(
                    f"btool failed with return code {result.returncode}: {result.stderr}"
                )
                if self.verbose:
            ...
            self.log_warning(
                f"Error running btool: {e}, falling back to manual parsing"
            )
            return self._parse_server_conf_manual()
    ...
    def _parse_server_conf_manual(self) -> Dict:
        ...
        # Also check default
        server_conf_default = (
            self.splunk_home / "etc" / "system" / "default" / "server.conf"
        )
    ...
    def _parse_outputs_conf_manual(self) -> Dict:
        ...
        outputs_conf_path = (
            self.splunk_home / "etc" / "system" / "local" / "outputs.conf"
        )
        outputs_conf_default = (
            self.splunk_home / "etc" / "system" / "default" / "outputs.conf"
        )
        ...
        for section_name in config.sections():
            ...
            self.log_debug(
                f"Manual parsing found section: [{section_name}] with {len(result[section_name])} keys"
            )

        self.log_debug(
            f"Manual parsing completed: {len(result)} sections total: {list(result.keys())}"
        )
        return result
    ...
    def _run_openssl_command(self, cmd_args: List[str]) -> Tuple[bool, str, str]:
        ...
        try:
            ...
            if splunk_path.exists():
                ...
                self.log_debug(
                    f"Using Splunk's bundled OpenSSL: {splunk_path} cmd openssl"
                )
            else:
                # Fallback to system OpenSSL if Splunk binary doesn't exist
                self.log_warning(
                    "Splunk binary not found, falling back to system OpenSSL"
                )
    ...
    def _verify_certificate_chain_openssl(
        self, server_cert_path: str, ca_cert_path: str
    ) -> bool:
    ...
    def _load_certificate_openssl(self, cert_path: str):
        ...
        if success:
            ...
            self.log_error(
                f"OpenSSL certificate verification failed for {cert_path}: {stderr}"
            )
            return None
            ...
            if b"-----BEGIN CERTIFICATE-----" in ca_data:
                ...
                for i, block in enumerate(
                    ...
                    try:
                        ...
                        self.log_warning(
                            f"Could not load certificate {i} from {ca_path}: {e}"
                        )
            else:
                ...
                for ca_cert in certificates:
                    try:
                        ...
                        if server_cert.issuer == ca_cert.subject:
                            self.log_debug(
                                f"Found matching CA certificate by issuer/subject"
                            )
                            return ca_cert
                    except Exception:
                        continue
        ...
        try:
            ...
            for name in san_ext:
                if isinstance(name, x509.IPAddress):
                    if name.value == ipaddress.IPv4Address(
                        "127.0.0.1"
                    ) or name.value == ipaddress.IPv6Address("::1"):
                        return True
    ...
    def verify_certificate_chain(self, cert, ca_cert) -> bool:
        ...
        if not HAS_CRYPTOGRAPHY:
            ...
            self.log_warning(
                "Certificate chain verification skipped (no valid paths for OpenSSL)"
            )
            return True

        try:
            # Check if this is the same certificate (self-signed case)
            if cert.fingerprint(hashes.SHA256()) == ca_cert.fingerprint(
                hashes.SHA256()
            ):
        ...
        try:
            ...
            if isinstance(public_key, rsa.RSAPublicKey):
                ...
                if "sha256" in signature_algorithm.lower():
                    ...
                    self.log_warning(
                        f"Unsupported hash algorithm: {signature_algorithm}"
                    )
                    return False
                ...
                # Use ECDSA algorithm
                public_key.verify(
                    cert.signature, cert.tbs_certificate_bytes, ec.ECDSA(hash_algorithm)
                )
            else:
    ...
    def get_config_value(
        self, config: Dict, stanza: str, key: str, default_value: str = None
    ) -> str:
        ...
        splunk_defaults = {
            ...
            "tcpout": {"compressed": "true", "useClientSSLCompression": "true"},
        }
        ...
        if (
            "::" in stanza
            and stanza.startswith("tcpout::")
            and stanza.split("::")[0] in config
        ):
            parent_stanza = stanza.split("::")[0]
            if key in config[parent_stanza]:
                value = config[parent_stanza][key]
                self.log_debug(
                    f"Found inherited value from {parent_stanza}.{key} = {value}"
                )
                return value
        ...
        if default_value is not None:
            self.log_debug(
                f"Using provided default for {stanza}.{key} = {default_value}"
            )
    ...
    def check_outputs_compression(self, outputs_conf: Dict) -> None:
        ...
        compression_settings = {"compressed": "true", "useClientSSLCompression": "true"}
    ...
    def verify_ssl_config_section(
        self, config: Dict, section_name: str = "sslConfig"
    ) -> Dict:
        ...
        ssl_compression = self.get_config_value(
            config, section_name, "allowSslCompression"
        )
        if ssl_compression and ssl_compression.lower() == "true":
            ...
            self.log_error(
                f"allowSslCompression must be set to true in [{section_name}] (current: {ssl_compression})"
            )

        # Check SSL renegotiation setting
        ssl_renegotiation = self.get_config_value(
            config, section_name, "allowSslRenegotiation"
        )
        if ssl_renegotiation and ssl_renegotiation.lower() == "true":
            results["ssl_renegotiation_ok"] = True
            self.log_success(
                f"allowSslRenegotiation is set to true in [{section_name}]"
            )
        else:
            self.log_error(
                f"allowSslRenegotiation must be set to true in [{section_name}] (current: {ssl_renegotiation})"
            )
        ...
        if server_cert_path:
            ...
            self.log_info(
                f"Checking server certificate: {server_cert_path} -> {full_cert_path}"
            )
            ...
            if os.path.exists(full_cert_path):
                ...
                if cert:
                    ...
                    # Verify against CA
                    ca_cert_path = ssl_config.get(
                        "caCertFile", ssl_config.get("sslRootCAPath", "")
                    )
                    if ca_cert_path:
                        ca_full_path = self.resolve_path(ca_cert_path)

                        self.log_info(
                            f"Checking CA certificate: {ca_cert_path} -> {ca_full_path}"
                        )
                        ...
                        if os.path.exists(ca_full_path):
                            ...
                            if ca_cert:
                                ...
                                if self.verify_certificate_chain(cert, ca_cert):
                                    ...
                                    self.log_success(
                                        f"Server certificate chain verification passed for [{section_name}]"
                                    )
                                else:
                                    self.log_error(
                                        f"Server certificate chain verification failed for [{section_name}]"
                                    )
                            else:
                                self.log_error(
                                    f"Could not load CA certificate from {ca_full_path}"
                                )
                        else:
                            self.log_error(
                                f"CA certificate file not found: {ca_full_path}"
                            )
                    else:
                        self.log_warning(
                            f"No CA certificate path specified in [{section_name}]"
                        )
    ...
    def verify_kvstore_section(self, config: Dict) -> Dict:
        ...
        if "kvstore" not in config:
            self.log_info(
                "[kvstore] section not found in server.conf - checking default certificates for KV Store compatibility"
            )
            ...
            if "sslConfig" in config:
                ...
                if server_cert_path:
                    ...
                    if os.path.exists(full_cert_path):
                        ...
                        if cert:
                            self.log_info(
                                "Evaluating default SSL certificate for KV Store compatibility"
                            )
                            results["server_cert_exists"] = True
                            results["server_cert_valid"] = True
                            ...
                            if purposes["no_purpose"] or (
                                purposes["server_auth"] and purposes["client_auth"]
                            ):
                                results["server_cert_purpose_ok"] = True
                                if purposes["no_purpose"]:
                                    self.log_success(
                                        "Default SSL certificate has no purpose restrictions (KV Store compatible)"
                                    )
                                else:
                                    self.log_success(
                                        "Default SSL certificate has dual purpose (KV Store compatible)"
                                    )
                            else:
                                # Default certificates are typically more lenient
                                results["server_cert_purpose_ok"] = True
                                self.log_info(
                                    "Default SSL certificate purpose is acceptable for KV Store (no custom restrictions)"
                                )
                            ...
                            if san_ok:
                                ...
                                self.log_success(
                                    "Default SSL certificate SAN contains localhost (KV Store compatible)"
                                )
                            else:
                                # For default certificates, this might be OK if using default KV Store settings
                                results["server_cert_san_ok"] = True
                                self.log_info(
                                    "Default SSL certificate SAN: KV Store will use default verification behavior"
                                )
                            ...
                            if ca_cert_path:
                                ...
                                if os.path.exists(ca_full_path):
                                    ...
                                    if ca_cert:
                                        ...
                                        ca_purposes = self.check_certificate_purpose(
                                            ca_cert
                                        )
                                        if ca_purposes["no_purpose"] or (
                                            ca_purposes["server_auth"]
                                            and ca_purposes["client_auth"]
                                        ):
                                            results["ca_cert_purpose_ok"] = True
                                            self.log_success(
                                                "Default CA certificate purpose is KV Store compatible"
                                            )
                                        else:
                                            results["ca_cert_purpose_ok"] = (
                                                True  # Default CA certs are generally OK
                                            )
                                            self.log_info(
                                                "Default CA certificate is acceptable for KV Store"
                                            )

                                        if self.verify_certificate_chain(cert, ca_cert):
                                            results["cert_chain_valid"] = True
                                            self.log_success(
                                                "Default certificate chain verification passed for KV Store"
                                            )
                                        else:
                                            # Be more lenient with default certificate chains
                                            results["cert_chain_valid"] = True
                                            self.log_info(
                                                "Default certificate chain: KV Store should work with default configuration"
                                            )
                                else:
                                    self.log_warning(
                                        f"Default CA certificate file not found: {ca_full_path}"
                                    )
                            else:
                                self.log_warning(
                                    "No CA certificate path found in default SSL configuration"
                                )
                        else:
                            self.log_warning(
                                f"Could not load default SSL certificate from {full_cert_path}"
                            )
                    else:
                        self.log_warning(
                            f"Default SSL certificate file not found: {full_cert_path}"
                        )
                else:
                    self.log_warning("No serverCert found in [sslConfig] section")
            else:
                self.log_warning(
                    "No [sslConfig] section found - cannot evaluate default certificates"
                )
            return results
        ...
        verify_server_name = self.get_config_value(
            config, "kvstore", "verifyServerName", "false"
        )
            ...
            if os.path.exists(full_cert_path):
                ...
                if cert:
                    ...
                    if purposes["no_purpose"] or (
                        ...
                        results["server_cert_purpose_ok"] = True
                        self.log_success(
                            "KV Store server certificate has correct purpose (no purpose or dual purpose)"
                        )
                    else:
                        self.log_error(
                            "KV Store server certificate must have no purpose or be dual purpose (client + server)"
                        )

                    # Check SAN for localhost (unless verifyServerName is disabled)
                    if results[
                        "verify_server_name_disabled"
                    ] or self.check_san_contains_localhost(cert):
                        results["server_cert_san_ok"] = True
                        self.log_success(
                            "KV Store server certificate SAN requirements satisfied"
                        )
                    else:
                        self.log_error(
                            "KV Store server certificate SAN must contain 127.0.0.1 or localhost (or disable verifyServerName)"
                        )

                    # Verify against CA
                    ca_cert_path = self.get_config_value(
                        config, "kvstore", "caCertFile"
                    ) or self.get_config_value(config, "kvstore", "sslRootCAPath")
                    if ca_cert_path:
                        ca_full_path = self.resolve_path(ca_cert_path)
                        if os.path.exists(ca_full_path):
                            ca_cert = self.load_ca_certificate(ca_full_path, cert)
                            if ca_cert:
                                ...
                                if ca_purposes["no_purpose"] or (
                                    ...
                                    results["ca_cert_purpose_ok"] = True
                                    self.log_success(
                                        "KV Store CA certificate has correct purpose"
                                    )
                                else:
                                    self.log_error(
                                        "KV Store CA certificate must have no purpose or be dual purpose"
                                    )

                                if self.verify_certificate_chain(cert, ca_cert):
                                    results["cert_chain_valid"] = True
                                    self.log_success(
                                        "KV Store certificate chain verification passed"
                                    )
                                else:
                                    self.log_error(
                                        "KV Store certificate chain verification failed"
                                    )
                            else:
                                self.log_error(
                                    f"Could not load KV Store CA certificate from {ca_full_path}"
                                )
                        else:
                            self.log_error(
                                f"KV Store CA certificate file not found: {ca_full_path}"
                            )
                    else:
                        self.log_warning(
                            "No CA certificate path specified for KV Store"
                        )
                else:
                    self.log_error(
                        f"Could not load KV Store server certificate from {full_cert_path}"
                    )
            else:
                self.log_error(
                    f"KV Store server certificate file not found: {full_cert_path}"
                )
        else:
            self.log_info(
                "No serverCert specified in [kvstore] - evaluating default certificates for KV Store compatibility"
            )
    ...
    def check_ca_completeness(self, config: Dict) -> bool:
        ...
        if "kvstore" in config:
            kvstore_config = config["kvstore"]
            ca_path = kvstore_config.get(
                "sslRootCAPath", kvstore_config.get("caCertFile", "")
            )
            if ca_path:
                ca_paths.add(self.resolve_path(ca_path))
        ...
        for ca_path in ca_paths:
            if not os.path.exists(ca_path):
                ...
                try:
                    ...
                    cert_count = content.count("-----BEGIN CERTIFICATE-----")
                    self.log_debug(
                        f"CA file {ca_path} contains {cert_count} certificate(s)"
                    )
    ...
    def check_version_compatibility(self) -> bool:
        ...
        if not self.splunk_version:
            self.log_warning(
                "Cannot verify version-specific requirements without Splunk version"
            )
            return False
        ...
        try:
            ...
            if (
                major > 9
                or (major == 9 and minor > 4)
                or (major == 9 and minor == 4 and patch >= 3)
            ):
                self.log_success(
                    f"Splunk version {self.splunk_version} supports custom KV Store certificates"
                )
                return True
            else:
                self.log_info(
                    f"Splunk version {self.splunk_version} - custom KV Store certificates have limited support"
                )
                self.log_info(
                    "For versions before 9.4.3, using default certificates is recommended"
                )
    ...
    def run_verification(self) -> Dict:
        ...
        print(
            f"{Colors.BOLD}Splunk KV Store Certificate Verification Tool{Colors.ENDC}"
        )
