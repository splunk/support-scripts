...
while true
do
    # Read process-runner PID (second line in splunkd.pid)
    ...
    pid=$(head -2 "$SPLUNK_HOME/var/run/splunk/splunkd.pid" 2>/dev/null | tail -1)
    ...
    if [ "$threads" -gt 500 ]; then
        output_file="$OUTPUT_DIR/pstack_process-runner-$pid-$threads-$(date +%s).out"
